...
const GoogleDriveAuth = () => {
  ...
  const handleAuthClick = async () => {
    ...
    const redirectUri = 'http://127.0.0.1:8000/dashboard';  // URL donde el usuario es redirigido después de autenticarse
  ...
  const extractTokenFromUrl = async () => {
    ...
    if (code) {
      try {
        ...
        const response = await axios.get(`http://127.0.0.1:8000/auth/exchange-token?code=${code}`);
