...
{
    \Log::info("Intentando eliminar KPI con UUID: " . $uuid);
    
    try {
        $kpiReport = KpiReport::where('id', $uuid)->firstOrFail();
        $kpiReport->delete();

        \Log::info("Informe de KPI eliminado correctamente");
        return response()->json(['message' => 'Informe de KPI eliminado correctamente'], 200);
    } catch (ModelNotFoundException $e) {
        \Log::error("KPI no encontrado: " . $uuid);
        return response()->json(['error' => 'Informe de KPI no encontrado'], 404);
    } catch (Exception $e) {
        \Log::error("Error al eliminar KPI: " . $e->getMessage());
